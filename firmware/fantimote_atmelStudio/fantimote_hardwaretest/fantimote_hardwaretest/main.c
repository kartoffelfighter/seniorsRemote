...
#include "storage.h"	// storage and register definitions
#include "config.h" // include config file
#include "main.h" // main header file

#ifdef WSLED
	#include "wsLED.h" // ws8212b led feedback led relevant code
#endif
...
ISR(INT0_vect){		// interrupt zero
	#if(UART >= 80)
	printf("INT0 triggered!");
	#endif
}
ISR(INT1_vect){		// interrupt one
	#if(UART >= 80)
	printf("INT1 triggered!");
	#endif
}
ISR(TIMER2_COMPA_vect){		// timer interrupt vector, called if 250µS have passed
	runtimeLow += 250;
	if(runtimeLow >= 1000){
		if(runtimeLow <= 1250){
			runtimeHigh++;
...
int main(void)
{
	...
	stdout = &str_uart;
		...
		char sprintf_output[80] = {0};
		sprintf(sprintf_output,"\t UART Output level: %d \n", UART); //printf(UART); printf("\n");
		printf(sprintf_output);
	#endif


	#ifdef WSLED
		#if UART >= 80	
			printf("testing ws8212b led: \n");
			printf("\t set to: white \n");
			led[0].r = 255;
			led[0].g = 255;
			led[0].b = 255;
	...
	while (1)	// loop
	{
		...
		if(runtimeHigh - prev >= 10000) {		
			volatile char heartbeat[20];
			sprintf(heartbeat, "%081X heartbeat \n", runtimeHigh - prev);
				printf(heartbeat);
			prev = runtimeHigh;
		}
		#endif
